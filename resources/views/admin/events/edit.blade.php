            ...
            <form method="POST" action="{{ route('events.update', $event->id) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input id="event_date" type="date" name="event_date"
                                value="{{ old('event_date', $event->event_date instanceof \Carbon\Carbon ? $event->event_date->format('Y-m-d') : $event->event_date) }}"
                                required
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input id="registration_deadline" type="date" name="registration_deadline"
                                value="{{ old('registration_deadline', $event->registration_deadline instanceof \Carbon\Carbon ? $event->registration_deadline->format('Y-m-d') : $event->registration_deadline) }}"
                                required
                                class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all text-gray-700" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input id="venue" type="text" name="venue"
                                value="{{ old('venue', $event->venue) }}" placeholder="Venue name or address" required
                                class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all placeholder:text-gray-400" />
                        </div>
                ...
                <div class="w-full lg:w-[300px] flex flex-col gap-6 shrink-0 lg:sticky lg:top-[100px]">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            @foreach ($covers as $key => $data)
                                <label class="cursor-pointer relative group">
                                    <!-- pre-select the event's current cover_style -->
                                    <input type="radio" name="cover_style" value="{{ $key }}"
                                        class="peer hidden"
                                        {{ old('cover_style', $event->cover_style) == $key ? 'checked' : '' }}>
                    ...
                    <div class="bg-white border border-gray-100 rounded-[24px] shadow-sm p-6">
                        ...
                        <p class="text-[11px] text-gray-500 mt-1 mb-6 leading-relaxed">Your updates will be visible
                            to attendees immediately.</p>
