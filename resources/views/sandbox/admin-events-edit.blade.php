@php
    ...
    $event = (object) [
        ...
        'category_id' => 1,
        'description' => 'A gathering of the brightest minds in technology. Join us for keynote speeches, workshops, and networking opportunities.',
        ...
        <main class="max-w-[1000px] mx-auto px-6 mt-10">
            ...
            <div class="mb-8 flex justify-between items-end">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Update the details for {{ $event->title }}.</p>
                </div>
            </div>
            ...
            <form method="POST" action="#" class="flex flex-col lg:flex-row gap-8 items-start">
                @csrf
                @method('PUT') <div class="flex-1 w-full bg-white border border-gray-100 rounded-[24px] shadow-sm p-8 space-y-6">
                    
                    <div class="space-y-1.5">
                        <label for="title" class="block text-xs font-semibold text-gray-700">Title</label>
                        <input id="title" type="text" name="title" value="{{ old('title', $event->title) }}" placeholder="e.g. Vento Hackathon 2026" required 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input id="event_date" type="date" name="event_date" value="{{ old('event_date', $event->event_date) }}" required 
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input id="end_date" type="date" name="end_date" value="{{ old('end_date', $event->event_date) }}" required 
                                class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all text-gray-700" />
                        </div>
                        <div class="space-y-1.5">
                            <label for="start_time" class="block text-xs font-semibold text-gray-700">Time</label>
                            <input id="start_time" type="time" name="start_time" value="{{ old('start_time', $event->start_time) }}" required 
                                class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all text-gray-700" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input id="maximum_slots" type="number" name="maximum_slots" value="{{ old('maximum_slots', $event->maximum_slots) }}" required min="1"
                                class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all text-gray-700" />
                        </div>
                        <div class="space-y-1.5">
                            <label for="registration_deadline" class="block text-xs font-semibold text-gray-700">Registration Deadline</label>
                            <input id="registration_deadline" type="date" name="registration_deadline" value="{{ old('registration_deadline', $event->registration_deadline) }}" required 
                                class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all text-gray-700" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input id="venue" type="text" name="venue" value="{{ old('venue', $event->venue) }}" placeholder="Venue name or address" required 
                                class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all placeholder:text-gray-400" />
                        </div>
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                <select id="status" name="status" required class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all appearance-none bg-white text-gray-700">
                                    ...
                                    <option value="Active" {{ old('status', $event->status) == 'Active' ? 'selected' : '' }}>Active (Public)</option>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea id="description" name="description" rows="5" placeholder="Tell attendees what to expect..." required 
                            class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all placeholder:text-gray-400 resize-none">{{ old('description', $event->description) }}</textarea>
                    </div>
                ...
                <div class="w-full lg:w-[300px] flex flex-col gap-6 shrink-0 sticky top-[100px]">
                    ...
                    <div class="bg-white border border-gray-100 rounded-[24px] shadow-sm p-6">
                        ...
                        <label for="cover_image" class="border-2 border-dashed border-gray-200 rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:border-orange-400 hover:bg-orange-50/30 transition-all group">
                            ...
                            <span class="text-sm font-bold text-gray-700">Replace current image</span>
                            <span class="text-[11px] font-medium text-gray-400 mt-1">PNG, JPG up to 5MB</span>
                        </label>
                    </div>

                    <div class="bg-white border border-gray-100 rounded-[24px] shadow-sm p-6">
                        <h3 class="text-sm font-bold text-gray-900">Save Changes</h3>
                        <p class="text-[11px] text-gray-500 mt-1 mb-6 leading-relaxed">Your updates will be visible to attendees immediately.</p>
